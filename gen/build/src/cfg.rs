...
pub struct Cfg<'a> {
    ...
    /// See [`CFG.export_macro`][CFG#export_macro].
...
/// ## **`CFG.export_macro`**
///
/// Used to generate export declaration macro. On windows, this is required to provide visible cpp symbols in an output DLL.
///
/// ### Example
///
/// ```no_run
...
mod r#impl {
    ...
    struct CurrentCfg {
        ...
        doxygen: bool,
        export_macro: Option<InternedString>,
    }
    ...
    impl<'a> Drop for Cfg<'a> {
        fn drop(&mut self) {
            if let Cfg::Mut(cfg) = self {
                ...
                current.export_macro = export_macro.map(intern);
            } else {
                CONST_DEREFS.with(|derefs| derefs.borrow_mut().remove(&self.handle()));
            }
        }
